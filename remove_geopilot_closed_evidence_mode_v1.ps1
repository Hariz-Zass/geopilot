...
$Targets=@(
  ...
  "$Root\frontend\src\pages\TrackBWorkspacePage.tsx"
)

foreach($P in $Targets){ if(!(Test-Path $P)){ throw "Missing source file: $P" } }

$Stamp=Get-Date -Format "yyyyMMdd_HHmmss"
$Backup="$Root\artifacts\closed_evidence_mode_removal_v1_backup_$Stamp"
...
Write-Host "============================================================"
Write-Host "GeoPilot Closed Evidence Mode Removal V1"
Write-Host "REMOVE MODE ENTIRELY - preserve evidence grounding"
Write-Host "============================================================"

try {
  Write-Host "[1] Stage patch + verifier"
  Copy-Item "$Root\patch_geopilot_remove_closed_evidence_mode_v1.py" "$Root\backend\_remove_closed_evidence_mode_v1.py" -Force
  Copy-Item "$Root\verify_geopilot_remove_closed_evidence_mode_v1.py" "$Root\backend\_verify_remove_closed_evidence_mode_v1.py" -Force

  Write-Host "[2] Apply source removal"
  docker compose exec -T backend python /app/_remove_closed_evidence_mode_v1.py
  if($LASTEXITCODE-ne 0){ throw "Closed Evidence Mode source patch failed." }

  Write-Host "[3] Remove TRACK_B_COMPETITION_MODE from env files"
  ...
  Write-Host "[4] Python syntax checks"
  ...
  Write-Host "[6] Track B regressions"
  $Tests=@(
  ...
  Write-Host "[7] Planning/document regressions"
  foreach($T in @(
  ...
  Write-Host "[8] Closed-mode removal verification"
  docker compose exec -T backend python /app/_verify_remove_closed_evidence_mode_v1.py
  if($LASTEXITCODE-ne 0){ throw "Removal verification failed." }
  ...
  Write-Host "============================================================"
  Write-Host "CLOSED EVIDENCE MODE REMOVAL V1 PASS"
  Write-Host "============================================================"
  ...
  Copy-Item "$Backup\TrackBWorkspacePage.tsx" "$Root\frontend\src\pages\TrackBWorkspacePage.tsx" -Force
  ...
  Remove-Item "$Root\backend\_remove_closed_evidence_mode_v1.py" -Force -ErrorAction SilentlyContinue
  Remove-Item "$Root\backend\_verify_remove_closed_evidence_mode_v1.py" -Force -ErrorAction SilentlyContinue
}
