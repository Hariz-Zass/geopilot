...
Write-Host "============================================================"
Write-Host "GeoPilot Smart Organizer Format Coverage V1.3.2"
Write-Host "Repair after V1.3.1 anchor mismatch"
...
$test = Join-Path $root "backend\tests\test_track_b_smart_organizer_format_coverage_v1_3_2.py"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backup = Join-Path $root "artifacts\smart_organizer_format_coverage_v1_3_2_backup_$stamp"
...
if (Test-Path $test) {
    Copy-Item $test (Join-Path $backup "test_track_b_smart_organizer_format_coverage_v1_3_2.py") -Force
}
Write-Host "BACKUP: $backup"

function Restore-Backup {
    Copy-Item (Join-Path $backup "track_b_smart_intake.py") $target -Force
    $old = Join-Path $backup "test_track_b_smart_organizer_format_coverage_v1_3_2.py"
    if (Test-Path $old) {
...
try {
    Write-Host "[0] Confirm V1.3 baseline + V1.3.1 rollback"
    ...
    foreach ($needle in @(
        ...
        if (-not $src.Contains($needle)) {
            throw "Expected audited V1.3 source marker missing: $needle"
        }
    }

    if ($src.Contains("SMART_ORGANIZER_FORMAT_COVERAGE_V1_3_2")) {
        throw "V1.3.2 already present. Stop and inspect."
    }
    ...
    if (-not $src.Contains($oldSuffix)) {
        throw "V1.3 ZIP suffix contract differs from audited source."
    }
...
'@
    ...
    if (-not $src.Contains($oldMapInfo)) {
        throw "V1.3 GIS bundle set contract differs from audited source."
    }
    $src = $src.Replace($oldMapInfo, $newMapInfo)

    Write-Host "[3] Add GPKG/QMD post-classification at exact V1.3 counts anchor"
    $countsAnchor = @'
    counts = {}
    for item in expanded:
...
'@
    ...
    $coverageBlock = @'
    # SMART_ORGANIZER_FORMAT_COVERAGE_V1_3_2
...
'@
    ...
    if (-not $src.Contains($countsAnchor)) {
        throw "Exact V1.3 counts anchor not found. STOP rather than patch unknown source."
    }
    $src = $src.Replace($countsAnchor, $coverageBlock)

    Set-Content $target $src -Encoding UTF8

    Write-Host "[4] Install focused static/runtime tests"
    @'
from pathlib import Path


def test_v132_static_contract():
    text = Path("/app/app/services/track_b_smart_intake.py").read_text()
    assert "SMART_ORGANIZER_FORMAT_COVERAGE_V1_3_2" in text
    assert '".ind"' in text
...
def test_mapinfo_ind_remains_optional():
    text = Path("/app/app/services/track_b_smart_intake.py").read_text()
    assert '_mapinfo_required = {".tab", ".dat", ".map", ".id"}' in text
    assert '_mapinfo_optional = {".ind"}' in text


def test_no_migration_or_db_write_added_by_v132():
    text = Path("/app/app/services/track_b_smart_intake.py").read_text()
    block = text.split("SMART_ORGANIZER_FORMAT_COVERAGE_V1_3_2", 1)[1]
    assert "alembic upgrade" not in block
    assert "session.add(" not in block
...
'@ | Set-Content $test -Encoding UTF8
    ...
    Write-Host "[6] Smart Organizer focused regression"
    docker compose exec -T backend python -m pytest -q `
        ...
        tests/test_track_b_smart_organizer_format_coverage_v1_3_2.py
    ...
    docker compose exec -T backend python -c "from pathlib import Path; t=Path('/app/app/services/track_b_smart_intake.py').read_text(); assert 'SMART_ORGANIZER_FORMAT_COVERAGE_V1_3_2' in t; assert '_mapinfo_optional = {\".ind\"}' in t; assert 'gis_geopackage_candidate' in t; assert 'organizer_auxiliary_ignored' in t; print('runtime_format_coverage_v1_3_2=PASS')"
    ...
    Write-Host "============================================================"
    Write-Host "SMART ORGANIZER FORMAT COVERAGE V1.3.2 PASS"
    Write-Host "============================================================"
