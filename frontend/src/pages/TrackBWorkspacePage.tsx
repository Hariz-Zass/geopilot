  ...
  return (
    <section className="trackb-command-center">
      <header className="trackb-hero">
        <div>
          <p className="trackb-kicker">PLAN-Ai Hackathon 2026 Â· Track B</p>
      ...
      {judgeMode && <section className="judge-command-deck">
        <div className="judge-title"><span>JUDGE VIEW Â· DECISION INTELLIGENCE</span><h2>One screen. Measured evidence â†’ planning action.</h2><p>GeoPilot separates deterministic satellite measurement from grounded AI reasoning so every recommendation remains traceable to organizer-supplied evidence.</p></div>
      ...
      {serverReadiness && <section className={`acceptance-gate ${serverReadiness.status}`}>
        ...
        <div className="acceptance-checks">{serverReadiness.checks.map((check) => <article key={check.key} className={check.status}><strong>{check.status === "pass" ? "âœ“" : check.status === "warn" ? "!" : "Ã—"} {check.label}</strong><span>{check.detail}</span></article>)}</div>
      ...
      <div className={`trackb-grid ${judgeMode ? "judge-hide-workbench" : ""}`}>
        <aside className="trackb-side-panel">
          <div className="panel-heading"><span>01</span><div><strong>Evidence Ingestion</strong><small>GeoTIFF Â· TIFF Â· JP2</small></div></div>
...
<form className="trackb-form" onSubmit={upload}>
            ...
            <button className="neon-button" disabled={busy}>{busy ? "Processingâ€¦" : "Ingest organizer evidence"}</button>
          </form>
          ...
          <div className="dataset-stack">
            ...
            <button className="analysis-button" disabled={busy || !beforeId || !afterId} onClick={() => void analyze()}>{busy ? "Analyzingâ€¦" : "Run temporal intelligence"}</button>
          </div>
        </aside>

        <main className="trackb-main-panel">
          <div className="temporal-preview-strip">
            <article><div><span>T1 Â· BEFORE</span><small>{datasets.find((d) => d.id === beforeId)?.acquisition_datetime ?? "Select dataset"}</small></div>{beforePreview ? <img src={beforePreview} alt="Before raster quicklook" /> : <div className="preview-placeholder">T1</div>}</article>
            <div className="temporal-arrow"><span>â†’</span><small>temporal delta</small></div>
            <article><div><span>T2 Â· AFTER</span><small>{datasets.find((d) => d.id === afterId)?.acquisition_datetime ?? "Select dataset"}</small></div>{afterPreview ? <img src={afterPreview} alt="After raster quicklook" /> : <div className="preview-placeholder">T2</div>}</article>
          </div>
          <div className="map-frame">
            <div className="map-hud">
              ...
              <span className="live-chip">context basemap Â· not evidence</span><span className="live-chip">deterministic</span>
            </div>
          ...
          <div className="metric-grid">
            {(result?.metrics ?? [
              { key: "change", label: "Changed pixels", value: "â€”", unit: "%" },
              { key: "area", label: "Changed area", value: "â€”", unit: "ha" },
              { key: "coverage", label: "Usable coverage", value: "â€”", unit: "%" },
          ...
          <div className="trackb-bottom-grid">
            <section className="intel-card ai-copilot-card"><div className="panel-heading compact"><span>AI</span><div><strong>GeoPilot Planning Copilot</strong><small>measurement first Â· grounded planning intelligence</small></div></div>
              {!aiInsight ? <><p>{result?.summary ?? "Run temporal analysis first. GeoPilot AI then converts measured satellite change into planner-focused issues, relevance and next actions without inventing spatial values."}</p>{result && <button className="analysis-button ai-run-button" onClick={() => void runAIInterpretation()} disabled={aiBusy}>{aiBusy ? "AI reasoningâ€¦" : "Generate planning intelligence"}</button>}</> : <div className="ai-intelligence"><div className="ai-confidence"><span>Grounding</span><strong>{aiInsight.confidence}</strong><small>{aiInsight.provider} Â· {aiInsight.model}</small></div><h3>{aiInsight.planner_problem}</h3><p>{aiInsight.executive_summary}</p>{aiInsight.insights.map((item, i) => <article className="ai-insight" key={`${item.title}-${i}`}><strong>{item.title}</strong><p>{item.finding}</p><small>PLANNING RELEVANCE</small><p>{item.planning_relevance}</p><small>NEXT MOVE</small><p>{item.recommended_action}</p><div className="ai-evidence-tags">{item.evidence_refs.map((ref) => <code key={ref}>{ref}</code>)}</div></article>)}<div className="ai-next-actions"><strong>Planner action queue</strong>{aiInsight.next_actions.map((x) => <p key={x}>â†’ {x}</p>)}</div></div>}
            ...
            <section className="intel-card"><div className="panel-heading compact"><span>EV</span><div><strong>Evidence Lineage</strong><small>auditable evidence sources</small></div></div>{result ? result.evidence.map((e, i) => <div className="evidence-row" key={i}><span>{String(e.role ?? "source")}</span><code>{String(e.id).slice(0, 12)}â€¦</code></div>) : <p>No analysis evidence yet.</p>}</section>
          </div>
          ...
          {result && <section className="planner-decision-workspace">
            <div className="decision-workspace-head">
              ...
              <div className="decision-question"><label>Planner problem / question<textarea value={plannerQuestion} onChange={(e) => setPlannerQuestion(e.target.value)} placeholder="Example: Which measured change should I inspect first, and what should I verify before escalating it?" rows={3} /></label><button className="analysis-button" onClick={() => void runPlannerDecision()} disabled={aiBusy}>{aiBusy ? "GeoPilot reasoningâ€¦" : "Build decision brief"}</button></div>
            </div>
            {terrainPlanningRun ? (
              <div className="decision-grid">
                ...
                <article className="decision-limit">
                  ...
                  {terrainPlanningRun.limitations.length ? (
                    ...
                    <p>
                      â€¢ Terrain values are deterministic measurements
                      from the selected Site DEM.
                    </p>
                  )}
                  <p>
                    â€¢ Professional planning interpretation remains
                    separate from the measured terrain values.
                  </p>
                </article>
              </div>
            ) : !decision ? <div className="decision-empty"><strong>Decision brief not generated yet</strong><span>GeoPilot will convert the deterministic temporal result into Issue â†’ Evidence â†’ Priority â†’ Planning implication â†’ Action, without treating spectral change as statutory proof.</span></div> : <div className="decision-grid">
              <article className={`decision-priority priority-${decision.priority}`}><small>NON-STATUTORY TRIAGE</small><strong>{decision.priority.replace("_", " ")}</strong><span>{decision.confidence} confidence</span><code>{decision.provider} Â· {decision.model}</code></article>
              ...
              <div className="decision-actions"><div className="decision-section-title">Recommended planner actions</div>{decision.recommended_actions.map((item, i) => <article key={`${item.action}-${i}`}><span>{String(i + 1).padStart(2, "0")}</span><div><strong>{item.action}</strong><p>{item.rationale}</p><small>VERIFY â†’ {item.verification_needed}</small><div className="ai-evidence-tags">{item.evidence_refs.map((ref) => <code key={ref}>{ref}</code>)}</div></div></article>)}</div>
              <article className="decision-limit"><small>LIMITATIONS / REVIEW BOUNDARY</small>{decision.limitations.map((item) => <p key={item}>â€¢ {item}</p>)}<p>â€¢ Professional review remains required; this priority is planner triage, not approval or compliance.</p></article>
            </div>}
          </section>}

          {result && <section className="limitations"><strong>Professional review boundary</strong>{result.limitations.map((x) => <p key={x}>â€¢ {x}</p>)}</section>}
      ...
      <section className="planner-decision-workspace hackathon-simulation">
        <div className="decision-workspace-head">
          ...
          <button className="analysis-button" onClick={() => void runHackathonSimulation()} disabled={hackathonBusy}>{hackathonBusy ? "Running full missionâ€¦" : "Run full Track B mission"}</button>
        </div>
        {hackathonWorkflow && <div className="decision-grid">
          <article className={`decision-priority priority-${hackathonWorkflow.status === "complete" ? "monitor" : "evidence_limited"}`}><small>MISSION STATUS</small><strong>{hackathonWorkflow.status}</strong><span>{hackathonWorkflow.stages.filter((x) => x.status === "pass").length}/{hackathonWorkflow.stages.length} stages passed</span><code>{hackathonWorkflow.workflow_id.slice(0, 12)}â€¦</code></article>
          <article className="decision-core"><small>URBAN RESULT</small><h3>{hackathonWorkflow.urban_analysis.changed_percentage.toFixed(2)}% measured change</h3><p>{hackathonWorkflow.urban_decision?.planning_implication ?? hackathonWorkflow.urban_analysis.summary}</p><small>RURAL RESULT</small><p>{hackathonWorkflow.rural_analysis.changed_percentage.toFixed(2)}% measured change Â· {hackathonWorkflow.rural_decision?.planning_implication ?? hackathonWorkflow.rural_analysis.summary}</p></article>
          ...
          <div className="decision-actions"><div className="decision-section-title">Mission execution trace</div>{hackathonWorkflow.stages.map((stage, i) => <article key={stage.key}><span>{String(i + 1).padStart(2, "0")}</span><div><strong>{stage.label} Â· {stage.status.toUpperCase()}</strong><p>{stage.detail}</p></div></article>)}</div>
        </div>}
      </section>
      ...
      <section className="trackb-dataset-table">
        ...
        {datasets.map((d) => <div className="dataset-table-row" key={d.id}><span><strong>{d.name}</strong><small>{d.acquisition_datetime ?? "date not supplied"}</small></span><span>{label(d.provenance.location_type)} Â· {label(d.provenance.temporal_role)}</span><span>{d.band_names.join(", ")}</span><span>{d.width}Ã—{d.height}<small>{d.crs}</small></span><span className="verified">âœ“ organizer</span></div>)}
