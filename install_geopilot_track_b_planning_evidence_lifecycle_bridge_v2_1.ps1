...
'@
    ...
    $Block=$TText.Substring($Pos,$End-$Pos)
    $CallAnchor='        site_id=site_id,'
    if(-not $Block.Contains($CallAnchor)){ throw "execute_site_applicability call anchor not found." }
    $Block=$Block.Replace($CallAnchor,"        site_id=site_id,`n        site_state=site_state,")
    $TText=$TText.Substring(0,$Pos)+$Block+$TText.Substring($End)
